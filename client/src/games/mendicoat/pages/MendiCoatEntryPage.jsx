...
export default function MendiCoatEntryPage() {
  const { connectMC, setMCScreen, mcRoomId } = useMCStore();
  ...
  const handleCreate = async () => {
    ...
    if (!ok) return;
    setLoading(true);
    connectMC('', name.trim(), avatar || 'P', user?.id);
    setLoading(false);
  };
  ...
  const stats = [
    ...
    { value: '10s', label: 'mendis' },
    { value: 'trump', label: 'rang' },
  ];

  const features = [
    ['Capture 10s', 'Winning team needs more Mendis'],
    ['Team play', 'Strategic partnerships (2v2)'],
  ];
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, marginBottom: 22 }}>
          <div>
            ...
            <p style={{ margin: '6px 0 0', fontSize: '0.78rem', color: 'var(--muted)' }}>
              Capture the 10s · Strategic Team Card Game
            </p>
          </div>
