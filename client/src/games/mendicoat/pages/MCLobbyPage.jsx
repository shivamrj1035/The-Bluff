...
export default function MCLobbyPage() {
  const {
    mcGameState, mcRoomId, mcStatus, mcDisconnect, mcStartGame,
    mcKickPlayer, mcReorderPlayers, mcSendChat, mcChatMessages,
    mcHostTransferredName, mcHostTransferredId, mcSocket, mcAddBot
  } = useMCStore();
  ...
  const players = mcGameState?.players || [];
  const connectedCount = players.filter(p => p.isConnected).length;
  const canStart = isHost && connectedCount === 4;

  // Host transfer toast
  useEffect(() => {
    if (!mcHostTransferredName) return;
    if (mcHostTransferredId === myId) toast.success('👑 You are now the host!');
    else toast.info(`👑 ${mcHostTransferredName} is the new host`);
  }, [mcHostTransferredId, mcHostTransferredName, myId]);

  // Error toast
  const { mcError } = useMCStore();
  useEffect(() => {
    if (mcError) toast.error(mcError);
  }, [mcError]);

  const copyInvite = () => {
    const url = `${window.location.origin}?game=mendicoat&room=${mcRoomId}`;
  ...
  return (
    <div style={{
      ...
      <div style={{ width:'100%', maxWidth:560, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        <div>
          <p style={{ margin:0, fontSize:'0.65rem', fontWeight:800, letterSpacing:'0.14em', color:'#fb923c' }}>MENDICOAT LOBBY</p>
          <h2 style={{ margin:'2px 0 0', fontSize:'1.4rem', fontWeight:900, color:'#fff' }}>
            Table <span style={{ color:'#fb923c', fontFamily:'monospace' }}>{mcRoomId}</span>
          </h2>
        </div>
        <div style={{ display:'flex', gap:8 }}>
          <button
            onClick={() => { mcDisconnect(); setScreen('LEADERBOARD'); }}
        ...
        <div style={{ padding:'20px 20px 0' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
            {players.map((p, idx) => {
              ...
              const isPlayerHost = p.id === mcGameState?.hostId;
              const chatMsg = mcChatMessages?.find(m => m.senderId === p.id);
                ...
                <div key={p.id} style={{ position:'relative', display:'flex', alignItems:'center', gap:12, padding:'10px 14px', borderRadius:14, background: isMe ? 'rgba(251,146,60,0.06)' : 'rgba(255,255,255,0.03)', border:`1px solid ${isMe ? 'rgba(251,146,60,0.18)' : 'rgba(255,255,255,0.05)'}` }}>
                  ...
                  {isHost && !isMe && (
                    <div style={{ display:'flex', flexDirection:'column', gap:2 }}>
                      {idx > 0 && (
                        <button onClick={() => { const ids = players.map(x => x.id); [ids[idx-1], ids[idx]] = [ids[idx], ids[idx-1]]; mcReorderPlayers(ids); }}
                      ...
                      {idx < players.length - 1 && (
                        <button onClick={() => { const ids = players.map(x => x.id); [ids[idx], ids[idx+1]] = [ids[idx+1], ids[idx]]; mcReorderPlayers(ids); }}
                          style={{ background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.08)', color:'#9ca3af', fontSize:'0.7rem', borderRadius:6, padding:'2px 6px', cursor:'pointer' }}>▼</button>
                      )}
                    </div>
                  )}
                  ...
                  {/* Kick button */}
                  {isHost && !isMe && (
                    <button onClick={() => mcKickPlayer(p.id)}
        ...
        <div style={{ padding:'16px 20px 20px', display:'flex', flexDirection:'column', gap:10 }}>
          ...
          {isHost ? (
            <motion.button
              ...
              onClick={canStart ? mcStartGame : undefined}
              style={{
            ...
            >
              {connectedCount < 4 ? `Waiting for players... (${connectedCount}/4)` : '🃏 Start MendiCoat'}
            </motion.button>
          ) : (
