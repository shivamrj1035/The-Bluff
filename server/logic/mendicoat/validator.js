const { MC_GAME_STATES, MAX_REDEALS } = require('./constants');
...
/**
 * Server-authoritative move validation for MendiCoat.
 * Prevents cheating: suit-following, turn order, card ownership.
 */
function validateMCAction(room, playerId, action, payload) {
  ...
  switch (action) {
    case 'MC_SELECT_TRUMP': {
      if (room.state !== MC_GAME_STATES.TRUMP_SELECTION) {
    ...
    case 'MC_PLAY_CARD': {
      if (room.state !== MC_GAME_STATES.PLAYING) {
...
module.exports = { validateMCAction };
